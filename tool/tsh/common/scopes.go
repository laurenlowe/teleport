...
func (c *scopesLSCommand) run(cf *CLIConf) error {
	...
	if err := tc.WithRootClusterClient(ctx, func(clt authclient.ClientI) error {
		var cursor string
		for {
			rsp, err := clt.ScopedAccessServiceClient().ListScopedRoleAssignments(ctx, &scopedaccessv1.ListScopedRoleAssignmentsRequest{
				PageToken: cursor,
	...
	if c.verbose {
		...
		SubAssignments:
			for _, subAssignment := range assignment.GetSpec().GetAssignments() {
				for _, row := range rows {
					if scopes.SortCmp(row.scope, subAssignment.GetScope()) == 0 {
		...
		slices.SortFunc(assignedScopes, scopes.SortCmp)
		assignedScopes = slices.CompactFunc(assignedScopes, func(a, b string) bool {
			return scopes.SortCmp(a, b) == 0
		})
