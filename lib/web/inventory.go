...
func (h *Handler) clusterUnifiedInstancesGet(w http.ResponseWriter, r *http.Request, p httprouter.Params, sctx *SessionContext, cluster reversetunnelclient.Cluster) (any, error) {
	...
	if servicesParam := values.Get("services"); servicesParam != "" {
		services := strings.Split(servicesParam, ",")
		filter.Services = make([]string, 0, len(services))
		for _, s := range services {
			if s = strings.TrimSpace(s); s != "" {
				filter.Services = append(filter.Services, s)
			}
		}
	}
	...
	if upgradersParam := values.Get("upgraders"); upgradersParam != "" {
		...
		filter.Upgraders = make([]string, 0, len(upgraders))
		for _, u := range upgraders {
			if u = strings.TrimSpace(u); u != "" {
				filter.Upgraders = append(filter.Upgraders, u)
			}
		}
	}

	if updaterGroupsParam := values.Get("updaterGroups"); updaterGroupsParam != "" {
		updaterGroups := strings.Split(updaterGroupsParam, ",")
		filter.UpdaterGroups = make([]string, 0, len(updaterGroups))
		for _, g := range updaterGroups {
			if g = strings.TrimSpace(g); g != "" {
				filter.UpdaterGroups = append(filter.UpdaterGroups, g)
			}
		}
	}
	...
	if typesParam := values.Get("types"); typesParam != "" {
		...
		filter.InstanceTypes = make([]inventoryv1.InstanceType, 0, len(types))
		for _, t := range types {
