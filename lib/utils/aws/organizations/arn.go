...
// accountARNBelongsToOrgID extracts the organization ID from an account ARN, and checks if it matches the provided organization ID.
// Example ARN: arn:aws:organizations::<org-master-account-id>:account/<org-id>/<account-id>
func accountARNBelongsToOrgID(accountARN string, organizationID string) error {
	return orgARNBelongsToOrgID(accountARN, organizationID)
}

// accountARNBelongsToOrgID extracts the organization ID from an account ARN, and checks if it matches the provided organization ID.
// Example ARN: arn:aws:organizations::<org-master-account-id>:root/<org-id>/<root-ou-id>
func rootOUARNBelongsToOrgID(rootOUARN string, organizationID string) error {
	return orgARNBelongsToOrgID(rootOUARN, organizationID)
}

func orgARNBelongsToOrgID(orgARN string, organizationID string) error {
