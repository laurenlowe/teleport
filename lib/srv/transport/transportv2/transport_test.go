...
type testPack struct {
	Client transportv2pb.TransportServiceClient
	Server *Service
}
...
func newServer(t *testing.T, cfg ServerConfig) testPack {
	...
	return testPack{
		Client: transportv2pb.NewTransportServiceClient(cc),
		Server: srv,
	}
}
...
// TODO(cthach): Add test for access denied due to MFA failed challenge.
func TestService_ProxySSH_Errors(t *testing.T) {
	...
	tests := []struct {
		...
		fn        func(t *testing.T, stream transportv2pb.TransportService_ProxySSHClient, conn *echoConn)
	}{
		{
			...
			fn: func(t *testing.T, stream transportv2pb.TransportService_ProxySSHClient, conn *echoConn) {
				require.NoError(t, stream.Send(&transportv2pb.ProxySSHRequest{}))
		...
		},
		{
			...
			fn: func(t *testing.T, stream transportv2pb.TransportService_ProxySSHClient, conn *echoConn) {
				err := stream.Send(&transportv2pb.ProxySSHRequest{
					Payload: &transportv2pb.ProxySSHRequest_DialTarget{
						DialTarget: &transportv2pb.TargetHost{
							HostPort: "1234",
							Cluster:  "test",
		...
		},
		{
			...
			fn: func(t *testing.T, stream transportv2pb.TransportService_ProxySSHClient, conn *echoConn) {
				...
				resp, err := stream.Recv()
				require.NoError(t, err)
				require.NotNil(t, resp.GetDetails())
				// require.Nil(t, resp.GetPayload())

				require.NoError(t, conn.Close())
				msg := []byte("hello")
				require.NoError(t, stream.Send(&transportv2pb.ProxySSHRequest{
					Payload: &transportv2pb.ProxySSHRequest_Ssh{
						Ssh: &transportv2pb.Frame{Payload: msg},
					},
				}))
...
// TODO(cthach): Update tests to handle MFA.
func TestService_ProxySSH(t *testing.T) {
	...
	agentStream := newClientStream(stream, func(payload []byte) *transportv2pb.ProxySSHRequest {
	...
	sshStream := newClientStream(stream, func(payload []byte) *transportv2pb.ProxySSHRequest {
		return &transportv2pb.ProxySSHRequest{
...
type clientStream struct {
	...
	responseFn func(payload []byte) *transportv2pb.ProxySSHRequest
}

func newClientStream(stream transportv2pb.TransportService_ProxySSHClient, responseFn func(payload []byte) *transportv2pb.ProxySSHRequest) *clientStream {
	return &clientStream{
