...
impl RdpDecoder {
    ...
    pub fn process(&mut self, tdp_fast_path_frame: &[u8]) {
        ...
        for update in updates {
            match update {
                UpdateKind::None => {
                    println!("Got a no update")
                }
                UpdateKind::Region(inclusive_rectangle) => {
                    println!("Got a region update: {:?}", inclusive_rectangle);

                    if inclusive_rectangle.right > self.image.width()
                        || inclusive_rectangle.bottom >= self.image.height()
                    {
                        println!("Region exceeds bounds, needs resize!")
                        // TODO
                    }
                }
                UpdateKind::PointerDefault => {
                    println!("Pointer reset to system default")
                }
                ...
                UpdateKind::PointerBitmap(decoded_pointer) => {
                    println!(
                        "Got bitmap data {}x{}",
                        decoded_pointer.width, decoded_pointer.height,
                    );
                }
            }
        }
