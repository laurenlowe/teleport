		...
		for {
			...
			case desktops := <-watcher.ResourcesC:
				...
				for _, dynamicDesktop := range desktops {
					...
					// TODO: can we use a long expiry here since the reconciler does explicit deletion?
					desktop.SetExpiry(s.cfg.Clock.Now().Add(apidefaults.ServerAnnounceTTL))
					s.cfg.Logger.WarnContext(ctx, "reconciler discovered new resource from watcher", "name", dynamicDesktop.GetName())
