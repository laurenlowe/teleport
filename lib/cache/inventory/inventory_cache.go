...
var (
	...
	unifiedExpressionParserErr  error
)
...
func (u *inventoryInstance) getVersionKey() bytestring {
	...
	if v.PreRelease == "" {
		return bytestring(ordered.Encode(v.Major, v.Minor, v.Patch, ordered.Inf, string(v.Metadata), name, id))
	}
	...
	encodeArgs = append(encodeArgs, string(v.Metadata))
...
func (ic *InventoryCache) initializeAndWatch(ctx context.Context) error {
	...
	primaryCacheSize := ic.cfg.PrimaryCache.GetApproximateCacheSize()
...
func (ic *InventoryCache) parseFilter(filter *inventoryv1.ListUnifiedInstancesFilter) (*parsedFilter, error) {
	...
	parser, err := getUnifiedExpressionParser()
	if err != nil {
		return nil, trace.Wrap(err)
	}
...
func getUnifiedExpressionParser() (*typical.Parser[*unifiedFilterEnvironment, bool], error) {
	unifiedExpressionParserOnce.Do(func() {
		...
		unifiedExpressionParser, unifiedExpressionParserErr = typical.NewParser[*unifiedFilterEnvironment, bool](spec)
	})

	return unifiedExpressionParser, unifiedExpressionParserErr
}
