...
type fakeProxySSHServer func(transportv2pb.TransportService_ProxySSHServer) error

// fakeServer is a [transportv2pb.TransportServiceServer] implementation
...
func (s fakeServer) ProxySSH(stream transportv2pb.TransportService_ProxySSHServer) error {
	return s.ssh(stream)
}
...
func TestClient_DialHost(t *testing.T) {
			...
			case req.GetDialTarget().GetCluster() == "payload-too-large":
				...
				if err := server.Send(&transportv2pb.ProxySSHResponse{
					Payload: &transportv2pb.ProxySSHResponse_Details{
						Details: &transportv2pb.ClusterDetails{FipsEnabled: true},
					},
				...
				case *transportv2pb.ProxySSHRequest_Ssh:
					if err := server.Send(&transportv2pb.ProxySSHResponse{
						Payload: &transportv2pb.ProxySSHResponse_Ssh{Ssh: &transportv2pb.Frame{Payload: bytes.Repeat([]byte{0}, 1001)}},
					}); err != nil && !errors.Is(err, io.EOF) {
						return trail.ToGRPC(trace.Wrap(err))
					}
				...
				return nil
			case req.GetDialTarget().GetCluster() == "echo":
				// send the initial cluster details
				if err := server.Send(&transportv2pb.ProxySSHResponse{
					Payload: &transportv2pb.ProxySSHResponse_Details{
						Details: &transportv2pb.ClusterDetails{FipsEnabled: true},
				...
				return nil
			case req.GetDialTarget().GetCluster() == "forward":
				// send the initial cluster details
				if err := server.Send(&transportv2pb.ProxySSHResponse{
					Payload: &transportv2pb.ProxySSHResponse_Details{
						Details: &transportv2pb.ClusterDetails{FipsEnabled: true},
				...
				agentStream := newServerStream(server, func(payload []byte) *transportv2pb.ProxySSHResponse {
					return &transportv2pb.ProxySSHResponse{Payload: &transportv2pb.ProxySSHResponse_Agent{Agent: &transportv2pb.Frame{Payload: payload}}}
				})
				...
				if err := server.Send(&transportv2pb.ProxySSHResponse{
					Payload: &transportv2pb.ProxySSHResponse_Ssh{Ssh: &transportv2pb.Frame{Payload: keys[0].Blob}},
				}); err != nil && !errors.Is(err, io.EOF) {
					return trail.ToGRPC(trace.Wrap(err))
				}
				return nil
	...
	tests := []struct {
		...
		assertion func(t *testing.T, conn net.Conn, details *transportv2pb.ClusterDetails, err error)
	}{
		{
...
type testPack struct {
	Client *Client
	Server transportv2pb.TransportServiceServer
}

// newServer creates a [grpc.Server] and registers the
// provided [transportv2pb.TransportServiceServer] with it opens
// an authenticated Client.
func newServer(t *testing.T, srv transportv2pb.TransportServiceServer) testPack {
	// gRPC testPack.
	...
	return testPack{
		Client: &Client{clt: transportv2pb.NewTransportServiceClient(cc)},
		Server: srv,
	}
}
...
func newServerStream(stream transportv2pb.TransportService_ProxySSHServer, responseFn func(payload []byte) *transportv2pb.ProxySSHResponse) *serverStream {
	return &serverStream{
